...
import qs from 'qs';  // 将对象转换为url拼接字符串
  ...
  loginRef.value.validate(async (valid) => {
    if (valid) {
      ...
      if (data.code == 200) {
        ...
        window.sessionStorage.setItem('currentUser', JSON.stringify(currentUser))  // JSON.parse()  json转js
        window.sessionStorage.setItem('menuList', JSON.stringify(data.menuList))  // JSON.parse()  json转js
...
function getCookie() {
  ...
  let decryptedPassword = loginForm.value.password;
  ...
  if (password !== undefined) {
    try {
      ...
      decryptedPassword = ""; // 解密失败时清空密码，避免泄露
    }
  }
